...
- **Inference & status** — `LabelInferenceService` derives the agent label (`codex`/`claude`/`gemini`/`aider`, else `Shell`) and a short summary from terminal text only (no remote calls, ever). `apply(_:to:)` respects the `isSummaryUserEdited`/`isAgentUserEdited` locks — once a user renames a field, inference stops touching it. `StatusService` maps activity into `running`/`idle`/`attentionNeeded`/`exited`.
